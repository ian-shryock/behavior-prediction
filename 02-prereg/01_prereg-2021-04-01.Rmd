...
Personality characteristics have been linked to a number of important life outcomes and behaviors. However, such studies of personality prediction are as limited as their measures and assumptions are. Emerging evidence suggests that people have unique personality structures and dynamics [@beck2020consistency; @beck2020idiographic], which suggests that using between-person measures of personality to test prediction questions misses the uniqueness of people in order to capture what is common. Such uniqueness may have it's own consequences for both short-term behaviors and long-term consequences.  

The present study asks the extent to which two behaviors -- procrastination and loneliness -- which have been linked to both short and long term well-being, academic and career success, and social relationships, can be predicted at the level of an individual.  
...
2b. Do these differences "cluster" according to the types of features used? In other words, for some people, are situations better predictors of procrastination and loneliness while for others psychological characteristics like personality and affect? Are still others best predicted by both?  
3. How much heterogeneity is there in the top predictors of loneliness and procrastination across people?  
...
Although the authors have accesses the data, the introduction, aims, hypotheses, and analysis plan in the proposed study were all written prior to any aggregation of the results. The data cleaning, analysis plan, and inferential tests will detailed carefully and all deviations from the planned analyses will be included the final manuscript.  

We have not run any analyses using such prediction models on the present data.  
...
To date, 215 participants have taken part in the study, and 210 of these participants additionally agreed to take part in the ESM portion of the study. We will use a subset of these participants with at least 40 responses, which we anticipate to be between 125 and 150, which is nearly triple the number of participants used in similar studies of idiographic prediction.  
...
- 10 affect indicators (positive, negative, neutral)  
- 21 binary situation indicators  
- 8 situation indicators from the DIAMONDS Situation-8  
...
Personality was measured in ESM surveys using the full BFI-2 (Soto \& John, 2017). For the purposes of the present study, items will be reversed scored and composited into their resulting 15 facets.  
...
3. Random Forest Models [@kim2019depression]  
...
- Personality indicators (15)  
- Affective indicators (8)  
- Binary situation indicators (19)  
- DIAMONDS situation indicators (8)  
- Psychological indicators (personality + affect) (23)  
- Situation indicators (binary + DIAMONDS) (27)  
- Full set (personality + affect + binary situations + DIAMONDS) (52)  
...
The statistical models used in the present study represent three machine learning classification methods -- [elastic net GLM](#elanet), [BISWIT](#biswit), and [Random Forest](#rfa) -- each of which were described in detail above.  
...
We will also transform our outcome variables -- loneliness and procrastination (0/1) -- to lagged outcomes, such that the features will then be predicting next time point loneliness or procrastination (t + 1) from current time point features (t).  
...
Inference criterion will be based on classification error and area under the receiver operating characteristic curve. Classification error is a simple estimate of the percentage of the test sample that was correctly classified by the model. In addition, the ROC or AUC will capture the trade-off between sensitivity and specificity across a threshold. In the the present study, we will use an AUC threshold of .5, which indicates binary classification at chance levels. ROC plots 1 - specificity (i.e. false positive rate: false positives / (false positives + true negatives)) against sensitivity (i.e. true positive rate: true positives / (true positives + false positives)).  
